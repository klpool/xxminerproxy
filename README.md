...
9. 中转端口可以开启`ws`加密模式，可以前置`CDN`/`Nginx`等任意的web反向代理，矿机端只需要运行标准的通过tcp转发ws协议即可连接中转端口，全程加密，防止被监控。
10. 中转端口可以开启`ssl/tls`加密模式，配置域名证书和密钥，全程加密，防止被监控。
11. 支持一切基于`Stratum v1 v2` 挖矿协议的`ETH`矿池，ssl/tls加密协议和tcp协议都支持。
...
执行下面的一键安装脚本，就把FXminer安装为了系统服务。

```shell
bash <(curl -s -L https://raw.githubusercontent.com/klpool/xxminerproxy/main/install.sh)
```
